...
class TrainerV41:
    ...
    async def _integrity_check_and_repair(self) -> None:
        ...
        for s in self.config.symbols:
            ...
            csv_p = self.config.storage_dir / f"{s}_1d.csv"
            if not p.exists() and not csv_p.exists():
                missing.append(s)
                continue
            try:
                df = pd.read_parquet(p) if p.exists() else pd.read_csv(csv_p)
                if df.empty:
                    missing.append(s)
            except Exception:
                missing.append(s)
        ...
        if not missing:
            self.logger.info("Integrity check OK: no missing parquet/csv in base_10y")
            return
        ...
        for sym, df in repaired.items():
            ...
            try:
                ...
                self.logger.warning("Failed to save parquet for %s, saving as CSV instead: %s", sym, e)
                out_csv = self.config.storage_dir / f"{sym}_1d.csv"
                normalized.to_csv(out_csv, index=False)
            self.logger.info("Repaired and wrote %s rows for %s", len(normalized), sym)

    @staticmethod
    def _normalize_dates(df: pd.DataFrame) -> pd.DataFrame:
        """Normalize Date column: handle mixed timezones, convert to UTC then strip tz."""
        df = df.copy()
    ...
    def _read_symbol_data(self, sym: str) -> pd.DataFrame | None:
        """Read symbol data from parquet (preferred) or CSV fallback."""
        # Try parquet first
        ...
        if parquet_path.exists():
            try:
                ...
                self.logger.warning("Parquet read failed for %s (%s), trying CSV...", sym, exc)

        # CSV fallback
        csv_path = self.config.storage_dir / f"{sym}_1d.csv"
        if csv_path.exists():
            try:
                df = pd.read_csv(csv_path)
                return self._normalize_dates(df)
            except Exception as exc:
                self.logger.warning("CSV read also failed for %s: %s", sym, exc)
