...
> 最後更新：2026-05-17 HKT（kiro-rewrite-loop scheduled auto-run）
>
...
| 檢查項 | 狀態 | 備註 |
|--------|------|------|
| Launcher PID | ✅ 運行中 | PID 27404，~12h uptime |
| OpenD 11112 | ✅ 開放 | |
| WebSocket 8787 | ✅ 開放 | |
| Dashboard 3000 | ✅ 開放 | |
| state.json / config.json | ✅ 合法 JSON | circuit_broken=False，consecutive_errors=0 |
| 回歸測試 | ✅ **597 passed, 1 skipped** | PRs #83-#91 新增 242+ 測試 |
| yfinance FD | ⚠️ **需重啟 launcher** | 1,401/1,463 FD 為 yfinance（PR #87+#90 已合併但 launcher 未重啟） |

---

## 4. PR 記錄（完整至 2026-05-17）
...
1. **重啟 launcher PID 27404**：啟用 PR #87（closing() 包裝器）+ PR #90（每 N symbol 重置 FD）。FD 計數應從 1,401 降至 <50。
2. **REAL 帳戶 281756460301136301 DISABLED** — broker 端，提 Futu 支援票，無需程式碼分支。
...
- 目前基線：**597 passed, 1 skipped**；任何改動不得使通過數下降。
...
*Generated by kiro-rewrite-loop scheduled auto-run — 2026-05-17*
