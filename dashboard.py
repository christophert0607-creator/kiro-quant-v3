...
RUNTIME_PROFILE_OPTIONS = ["micro", "lite", "standard"]
...
config = load_config()
v3_live_cfg = config.get("v3_live", {}) if isinstance(config, dict) else {}

st.sidebar.subheader("V3 Runtime")
current_runtime_profile = str(v3_live_cfg.get("runtime_profile", "lite")).lower()
if current_runtime_profile not in RUNTIME_PROFILE_OPTIONS:
    current_runtime_profile = "lite"
runtime_profile = st.sidebar.selectbox(
    "Runtime Profile",
    options=RUNTIME_PROFILE_OPTIONS,
    index=RUNTIME_PROFILE_OPTIONS.index(current_runtime_profile),
    help="對應 v3_launcher.py --profile（micro/lite/standard）",
)

st.sidebar.subheader("TSLA 設定")
tsla_buy = st.sidebar.number_input(
    "TSLA 買入價", value=float(config.get("TSLA", {}).get("buy_price", 380.0))
)
tsla_sell = st.sidebar.number_input(
    "TSLA 賣出價", value=float(config.get("TSLA", {}).get("sell_price", 410.0))
)

st.sidebar.subheader("TSLL 設定")
tsll_buy = st.sidebar.number_input(
    "TSLL 買入價", value=float(config.get("TSLL", {}).get("buy_price", 13.5))
)
tsll_sell = st.sidebar.number_input(
    "TSLL 賣出價", value=float(config.get("TSLL", {}).get("sell_price", 15.0))
)
...
if st.sidebar.button("保存設定"):
    ...
    config.setdefault("v3_live", {})["runtime_profile"] = runtime_profile
    save_config(config)
