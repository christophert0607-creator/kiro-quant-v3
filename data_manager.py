...
def _is_hk_symbol(symbol: str) -> bool:
    """Accept common HK symbol notations used across this repo."""
    if not isinstance(symbol, str):
        return False
    normalized = symbol.strip().upper()
    return (
        normalized.endswith(".HK")
        or normalized.startswith("HK.")
        or normalized.startswith("HKG.")
    )
...
class DataManager:
    ...
    def get_market_data(self, symbol, market="US"):
        ...
        if self.futu and _is_hk_symbol(symbol):
            try:
        ...
        if _get_east_money and _is_hk_symbol(symbol):
            try:
                # Extract HK code (e.g., "00700" from "00700.HK" or "HK.00700")
                normalized = symbol.strip().upper()
                hk_code = normalized.replace("HK.", "").replace("HKG.", "")
                if hk_code.endswith(".HK"):
                    hk_code = hk_code[:-3]
